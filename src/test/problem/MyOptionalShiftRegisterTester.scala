...
class MyOptionalShiftRegisterTests extends ChiselFlatSpec {
  ...
  backends foreach {backend =>
    it should s"correctly respond to inputs in $backend" in {
      Driver(() => new MyOptionalShiftRegister(2) , backend)(c => new MyOptionalShiftRegisterTester(c)) should be (true)
    }
  }
}                                                                                                                                    
...
iotesters.Driver.execute(args= Array("--backend-name","verilator")  , ()=> new MyOptionalShiftRegister(2)){
