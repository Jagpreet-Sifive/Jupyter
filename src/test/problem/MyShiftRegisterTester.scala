...
class MyShiftRegisterTests extends ChiselFlatSpec {
  ...
  backends foreach {backend =>
    it should s"correctly respond to inputs in $backend" in {
      Driver(() => new MyShiftRegister(2), backend)(c => new MyShiftRegisterTester(c)) should be (true)
    }
  }
}                                                                                                                                    
...
iotesters.Driver.execute(args= Array("--backend-name","verilator")  , ()=> new MyShiftRegister(2)){
