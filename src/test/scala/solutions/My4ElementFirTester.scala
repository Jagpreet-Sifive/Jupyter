...
class My4ElementFirTests extends ChiselFlatSpec {
  ...
  backends foreach {backend =>
    it should s"correctly respond to inputs in $backend" in {
      Driver(() => new My4ElementFir, backend)(c => new My4ElementFirTester(c)) should be (true)
    }
  }
}                                                                                                                                    
...
iotesters.Driver.execute(args= Array("--backend-name","verilator")  , ()=> new My4ElementFir){
